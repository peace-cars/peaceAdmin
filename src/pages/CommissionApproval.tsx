...
import { 
  Check, Clock, DollarSign, ShieldCheck, ArrowUpRight, Users, Activity, Star, FileCheck2, Lock
} from 'lucide-react';
...
export default function CommissionApproval() {
  ...
  const gmApprove = async (id: string) => {
    try {
      ...
      setCommissions(prev => prev.map(c => c.id === id ? { ...c, gmApproved: true, isPaid: true } : c));
    } catch (e) {
  ...
  const pendingPayoutTotal = commissions.filter(c => !c.isPaid).reduce((s, c) => s + Number(c.amountEtb), 0);
  const settledMtdTotal = commissions.filter(c => c.isPaid && new Date(c.createdAt).getMonth() === new Date().getMonth()).reduce((s, c) => s + Number(c.amountEtb), 0);
    ...
    <div className="space-y-10 pb-20 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        <Tooltip content="Total incentive volume pending verification">
          <KpiTile label="Pending Payout" value={`${(pendingPayoutTotal / 1000).toFixed(1)}K ETB`} icon={<Clock size={14} />} className="p-6 h-32" />
        </Tooltip>
        <Tooltip content="Total disbursements completed this month">
          <KpiTile label="Settled (MTD)" value={`${(settledMtdTotal / 1000).toFixed(1)}K ETB`} icon={<FileCheck2 size={14} />} className="p-6 h-32" color="emerald" />
        </Tooltip>
        <Tooltip content="Total network partners currently in registry">
          <KpiTile label="Active Partners" value={`${activePartnersCount} Units`} icon={<Users size={14} />} className="p-6 h-32" color="indigo" />
        </Tooltip>
        <Tooltip content="Percentage of payouts settled within SLA registry">
          <KpiTile label="SLA Compliance" value={slaCompliance} icon={<Star size={14} />} className="p-6 h-32" color="indigo" />
        </Tooltip>
      </div>
      ...
      <div className="space-y-6">
         <div className="flex flex-col gap-1.5 px-2">
            <div className="flex items-center justify-between">
               <h2 className="text-[13px] font-bold text-text-main font-semibold">Payout Authorization Pipeline</h2>
         ...
         {commissions.length === 0 ? (
          <div className="py-24 text-center rounded-2xl border border-dashed border-border-subtle bg-surface-card flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-text-muted/60 font-medium text-[13px]">No active incentives in registry.</p>
          </div>
        ) : (
             ...
             {commissions.map((c) => (
              <div 
                ...
                className={cn(
                  ...
                  c.isPaid && 'opacity-60 bg-bg-secondary/50 grayscale shadow-none border-dashed pointer-events-none'
                )}
              >
                ...
                <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0 hidden md:block">
                   <Badge variant="primary" className="shadow-lg">Authorize Settlement</Badge>
                </div>

                 <div className="flex gap-4 md:gap-6">
                  <div className={cn(
                    "w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center border transition-all shrink-0 shadow-inner",
                    c.isPaid ? 'bg-bg-secondary border-border-subtle/30 text-text-muted/30' : 
                    c.dmVerified ? 'bg-warning/10 border-warning/20 text-warning' :
                  ...
                  <div className="space-y-1 md:space-y-2 flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 md:gap-4">
                      <h3 className="text-sm md:text-base font-bold text-text-main tracking-tight leading-tight group-hover:text-primary-main transition-colors truncate">{c.beneficiaryName || 'Operational Partner'}</h3>
                    ...
                    {/* Horizontal Scroller for mobile tags */}
                     <div className="flex overflow-x-auto no-scrollbar gap-2 pt-1 md:pt-0 text-[13px] font-medium text-text-muted opacity-80 pb-1">
                      ...
                      <div className="shrink-0 flex items-center">
                        <span className="flex items-center gap-1.5 font-mono bg-bg-secondary px-2 py-1 rounded-md border border-border-subtle/30 text-text-muted/60"><Activity size={10} /> {new Date(c.createdAt).toLocaleDateString()}</span>
                      </div>
                      <div className="shrink-0 flex items-center">
                        <Badge variant="default" className="font-mono bg-bg-secondary border-border-subtle/30 text-text-muted/40">ID: {c.id.substring(0,8).toUpperCase()}</Badge>
                      </div>
                    </div>
                  </div>
                </div>
                 ...
                 <div className="flex flex-col md:flex-row md:items-center gap-5 md:gap-10 md:border-l border-border-subtle/30 md:pl-10 mt-2 md:mt-0 pt-4 md:pt-0 border-t border-border-subtle/30 md:border-t-0">
                  <div className="text-left md:text-right shrink-0 flex justify-between md:block items-end">
                    <div>
                      <p className="text-[13px] text-text-muted font-medium mb-0.5 md:mb-1.5">Authorization Sum</p>
                      <p className="text-2xl font-bold text-primary-main md:text-text-main tracking-tight leading-none">
                        ...
                        <span className="text-[13px] md:text-[11px] font-bold ml-1.5 uppercase opacity-80">ETB</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:flex items-center gap-3 w-full md:w-auto">
                    ...
                    <button 
                      ...
                      className={cn(
                        ...
                        (!c.dmVerified && role === 'DISTRICT_MANAGER') || (c.dmVerified && !c.gmApproved && (role === 'GENERAL_MANAGER' || role === 'FINANCE_AUDITOR')) ? "col-span-1 w-full md:w-11" : "col-span-2 w-full md:w-11"
                      )}
                    >
                        <span className="md:hidden font-medium text-[13px] flex items-center gap-2">Details <ArrowUpRight size={14} /></span>
       ...
       <div className="bg-surface-card border border-border-subtle/30 p-10 rounded-2xl shadow-sm flex items-center justify-between relative overflow-hidden group">
          <div className="flex items-center gap-16 relative z-10">
             <div className="space-y-1.5">
                <p className="text-[13px] font-bold text-text-muted/40 uppercase tracking-tight">Total Ledger Disbursements</p>
                <p className="text-2xl font-bold text-text-main tracking-tight leading-none">{(settledMtdTotal / 1000).toFixed(1)}K <span className="text-[11px] text-primary-main font-bold ml-1.5">ETB</span></p>
             </div>
             <div className="w-px h-12 bg-border-subtle/30" />
             <div className="space-y-1.5">
                <p className="text-[13px] font-bold text-text-muted/40 uppercase tracking-tight">Verification Protocol</p>
                <p className="text-2xl font-bold text-text-main tracking-tight leading-none">Verified <span className="text-[11px] text-success font-bold ml-2 font-medium">Active</span></p>
             </div>
          </div>
            ...
            <div className="space-y-8">
             <div className="flex items-center justify-between p-6 bg-bg-secondary rounded-2xl border border-border-subtle/30">
               <div className="flex items-center gap-4">
                 ...
                 <div>
                   <p className="text-lg font-bold text-text-main tracking-tight">{selectedCommission.beneficiaryName}</p>
                   <p className="text-[13px] font-bold text-text-muted/60">{selectedCommission.beneficiaryRole?.replace(/_/g, ' ')}</p>
                 </div>
               </div>
               <div className="text-right">
                 <p className="text-[13px] font-bold text-text-muted/40 mb-1">Approved Payout</p>
                 <p className="text-2xl font-bold text-text-main tracking-tight">{Number(selectedCommission.amountEtb).toLocaleString()} <span className="text-[13px] text-primary-main font-mono">ETB</span></p>
               </div>
             </div>
             
              <div className="space-y-4">
                <h4 className="text-[13px] font-bold text-text-muted/40 uppercase tracking-wider">Approval Timeline</h4>
                <div className="space-y-0 pl-4 border-l-2 border-border-subtle/30 ml-2">
                  <div className="relative pl-6 pb-6">
                    ...
                    <p className="text-[13px] text-text-muted/60 mt-1">{new Date(selectedCommission.createdAt).toLocaleString()}</p>
                  </div>
                  <div className={cn("relative pl-6 pb-6", !selectedCommission.dmVerified && "opacity-40")}>
                    <div className={cn("absolute w-3 h-3 rounded-full -left-[7px] top-1 shadow-md border-2 border-surface-card", selectedCommission.dmVerified ? "bg-warning" : "bg-border-subtle/50")} />
                    <p className="text-sm font-bold text-text-main leading-none">District Manager Verification</p>
                    <p className="text-[13px] text-text-muted/60 mt-1">
                      {selectedCommission.dmVerified ? "Verified locally" : "Pending DM Review"}
                    </p>
                  </div>
                  <div className={cn("relative pl-6", (!selectedCommission.gmApproved && !selectedCommission.isPaid) && "opacity-40")}>
                    <div className={cn("absolute w-3 h-3 rounded-full -left-[7px] top-1 shadow-md border-2 border-surface-card", selectedCommission.isPaid ? "bg-success" : "bg-border-subtle/50")} />
                    <p className="text-sm font-bold text-text-main leading-none">Final Audit & Settlement</p>
                    <p className="text-[13px] text-text-muted/60 mt-1">
                ...
                <Button variant="outline" className="h-11 rounded-xl text-[13px] font-bold" onClick={() => setSelectedCommission(null)}>Close Profile</Button>
                {!selectedCommission.dmVerified && role === 'DISTRICT_MANAGER' && (
                  <Button variant="primary" className="h-11 rounded-xl text-[13px] font-bold bg-warning text-bg" onClick={() => { dmVerify(selectedCommission.id); setSelectedCommission(null); }}>Verify Payout</Button>
                )}
                {selectedCommission.dmVerified && !selectedCommission.gmApproved && (role === 'GENERAL_MANAGER' || role === 'FINANCE_AUDITOR') && (
                  <Button variant="primary" className="h-11 rounded-xl text-[13px] font-bold bg-text-main text-bg" onClick={() => { gmApprove(selectedCommission.id); setSelectedCommission(null); }}>Settle Incentive</Button>
                )}
              </div>
           </div>
         )}
