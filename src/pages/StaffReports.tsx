...
export default function StaffReports() {
  ...
  const filteredMetrics = personnelMetrics.filter(
    (m) =>
      ...
      <div className="flex flex-col md:flex-row gap-4 md:items-center justify-between px-1">
        ...
        <div className="flex items-center gap-2 bg-bg-secondary/30 p-1.5 rounded-xl border border-border-subtle/30">
          <div className="px-4 py-2 bg-surface-card rounded-lg shadow-sm border border-border-subtle/30">
            <p className="text-[11px] text-text-muted/40 uppercase tracking-wider mb-0.5">
              Operational Capacity
            </p>
            <p className="text-sm font-bold text-text-main tracking-tight">
              {totalActions.toLocaleString()}{' '}
              <span className="text-[12px] opacity-30 ml-0.5">Units</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-6">
          <div className="flex items-center justify-between px-1">
            <h2 className="text-[12px] font-bold text-text-muted font-medium">
              Personnel Registry
            </h2>
            ...
            {loading ? (
              Array(6)
                ...
                .map((_, i) => (
                  <div
                    key={i}
                    className="bg-bg-secondary/30 h-20 rounded-xl animate-pulse border border-border-subtle/30"
                  />
                ))
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-6">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] text-text-muted/60 font-medium bg-bg-secondary px-2 py-0.5 rounded border border-border-subtle/30">
                            {staff.role?.replace(/_/g, ' ')}
                          </span>
                          <span className="text-[11px] text-text-muted/20 font-mono">
                            ID: {staff.id?.substring(0, 8).toUpperCase()}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex-grow max-w-[200px] space-y-2">
                      <div className="flex justify-between items-end">
                        <p className="text-[11px] text-text-muted/40 uppercase tracking-tight">
                          Load
                        </p>
                        <p className="text-[13px] font-bold text-text-main">
                          {((staff.totalSales / (totalActions || 1)) * 100).toFixed(1)}%
                        </p>
                      </div>
                    ...
                    <div className="text-left md:text-right shrink-0 px-8 border-l border-border-subtle/30">
                      <p className="text-[11px] text-text-muted font-medium mb-0.5 opacity-40">
                        Settled
                      </p>
                      <p className="text-base font-bold text-text-main tracking-tight leading-none">
                        {staff.totalSales || 0}
                      </p>
                    </div>
                  </div>
        ...
        <div className="space-y-8">
          <div className="space-y-4">
            ...
            <div className="bg-slate-900 rounded-2xl shadow-sm border border-slate-800 transition-all duration-300 text-white p-6 space-y-6 relative overflow-hidden">
              <div className="relative z-10 flex items-center gap-4">
                ...
                <div>
                  <p className="text-[11px] font-bold opacity-50 font-medium mb-0.5">
                    Efficiency Index
                  </p>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-surface-card rounded-2xl shadow-sm border border-border-subtle/30 transition-all duration-300 p-6 space-y-6">
              <div className="space-y-1">
                <p className="text-[11px] font-bold text-text-muted/40 uppercase tracking-tight">
                  Base Allocation
                </p>
                <p className="text-xl font-bold text-text-main tracking-tight leading-none">
                  12.5M{' '}
                  <span className="text-[12px] text-primary-main font-bold ml-0.5 font-medium">
                    ETB
                  </span>
                </p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xl font-bold text-success tracking-tight leading-none">
                  +840K <span className="text-[12px] font-bold ml-0.5 font-medium">ETB</span>
                </p>
              </div>
              <Button
                variant="outline"
                className="w-full h-11 rounded-lg border-border-subtle/30 text-[12px] font-bold hover:bg-bg-secondary transition-all active:scale-95"
              >
                Audit Log
              </Button>
            </div>
