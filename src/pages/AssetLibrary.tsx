  ...
  useEffect(() => {
    ...
    const fetchAssets = async () => {
      try {
        ...
        setAssets(allAssets.sort((a, b) => new Date(b.created).getTime() - new Date(a.created).getTime()));
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader 
        ...
        subtitle="Centralized archive for media and operational documentation."
        icon={<HardDrive size={18} className="text-primary-main" />}
        className="pb-6 border-b border-border-subtle/30"
      />

      <div className="flex flex-col md:flex-row gap-3 md:items-center justify-between">
        <div className="relative flex-grow max-w-sm">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted/30" size={14} />
          <input 
            type="text" 
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        {loading ? (
          Array(12).fill(0).map((_, i) => (
            <div key={i} className="aspect-square bg-bg-base/30 rounded-xl animate-pulse border border-border-subtle/30" />
          ))
          ...
          <div className="col-span-full py-24 text-center border border-dashed border-border-subtle/30 rounded-xl bg-bg-secondary">
             ...
             <p className="text-text-muted font-medium text-[11px] opacity-30">No archived assets found.</p>
          </div>
          ...
          >
             <div className="w-full aspect-square bg-bg-secondary border border-border-subtle/20 rounded-lg flex items-center justify-center text-primary-main mb-3 overflow-hidden relative shadow-inner">
                ...
                <div className="absolute inset-0 bg-text-main/0 hover:bg-text-main/10 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                    <button onClick={() => window.open(asset.url, '_blank')} className="p-2.5 bg-bg text-text-main rounded-lg shadow-xl transform scale-90 group-hover:scale-100 transition-all active:scale-95">
             ...
             <div className="w-full space-y-1">
                <h4 className="text-[12px] font-bold text-text-main tracking-tight uppercase line-clamp-1 group-hover:text-primary-main transition-colors px-1 leading-tight">{asset.name}</h4>
             ...
             <div className="absolute top-1.5 left-1.5 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity">
                <span className={cn(
                  ...
                  asset.category === 'FINANCE' ? "bg-success border-success text-bg" : "bg-text-main border-text-main text-bg"
                )}>
                   {asset.category}
                </span>
             </div>
