    ...
    try {
      ...
      const mapped = dataArray.map((v: any) => ({
        ...
        priceFormatted: v.retail_price_etb
          ? `${(Number(v.retail_price_etb) / 1000000).toFixed(1)}M ETB`
          : 'Price TBD',
    ...
    <div className="space-y-10 pb-20 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          ...
          <div className="w-full md:w-auto shrink-0 overflow-x-auto no-scrollbar pb-1 -mb-1">
            <div className="flex flex-col md:flex-row items-stretch md:items-center gap-2 md:gap-3 w-full md:w-auto">
              ...
              <Button
                variant="primary"
                className="h-12 px-6 md:px-8 shadow-lg shadow-primary-main/20 shrink-0 w-full md:w-auto text-sm md:text-base font-bold whitespace-nowrap"
                onClick={() => {
                  setEditingId(null);
                  setIsAdding(true);
                }}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Tooltip content="Total internal technical documents archived">
          <KpiTile
            label="Archives"
            value={inventory.reduce((s, v) => s + v.internalDocuments.length, 0)}
                ...
                <tr key={car.id} className="group transition-all">
                  ...
                  <td className="py-4 px-4 bg-bg-secondary/30 border-y border-border-subtle/30 group-hover:bg-bg-secondary/50 group-hover:border-primary-main/30 transition-all">
                    <div className="flex items-center gap-2">
                      <Tooltip
                        content={
                          car.gallery.length > 0 ? 'View Asset Photos' : 'No Photos Uploaded'
                        }
                      >
                        <div
                          className={cn(
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setFormData({
                              ...
                              status: String(car.status || 'SOURCING')
                                .replace(/ /g, '_')
                                .toUpperCase(),
          ...
          {filteredInventory.map((car) => (
            ...
            >
              ...
              <div className="w-14 h-14 rounded-lg overflow-hidden border border-border-subtle/30 bg-bg-secondary shrink-0 relative shadow-inner">
                ...
                />
                {car.floorPlanLoan && (
                  <div className="absolute inset-0 border-2 border-warning rounded-lg" />
                )}
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center">
                <div className="flex items-baseline justify-between gap-2">
                  <h3 className="text-xs font-bold text-text-main leading-tight tracking-tight truncate">
                    {car.year} {car.make}{' '}
                    <span className="font-medium text-text-muted">{car.model}</span>
                  </h3>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {car.certifiedKm && (
                    <div className="text-[9px] text-text-dim truncate">{car.certifiedKm} km</div>
                  )}
                </div>
              </div>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="outline" className="flex-1 h-12" onClick={() => setIsAdding(false)}>
              Cancel Operation
            </Button>
            <Button
              variant="primary"
