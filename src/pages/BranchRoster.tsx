...
export default function BranchRoster() {
  ...
  const uploadAvatar = async (staffId: string): Promise<string | null> => {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/storage/upload-base64`,
        {
          method: 'POST',
          headers: {
  ...
  const handleDeactivate = async (id: string) => {
    if (
      !window.confirm(
        'Deactivate this personnel record? They will lose access to the operational registry.',
      )
    )
      return;
    try {
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KpiTile
          label="Open Tasks"
          value={roster.reduce((s, m) => s + m.activeInspections, 0)}
              ...
              >
                ...
                <div className="flex items-start justify-between ml-2">
                  <div className="flex items-center gap-3">
                      ...
                      <div
                        className={cn(
                          ...
                          member.isOnline
                            ...
                            : 'bg-surface-card text-text-muted/30 border-border-subtle',
                        )}
                      >
                        {member.avatar ? (
                    ...
                    <div className="space-y-0.5">
                      <h3 className="text-sm font-bold text-text-main tracking-tight group-hover:text-primary-main transition-colors leading-none">
                        {member.fullName}
                      </h3>
                      <div className="flex items-center gap-1.5 mt-1">
                        <p className="text-primary-main text-[11px] font-medium leading-none">
                          {member.role?.replace(/_/g, ' ')}
                        </p>
                        <span className="w-1 h-1 bg-border-subtle rounded-full" />
                        <p className="text-text-muted text-[11px] font-medium leading-none">
                  ...
                  <div
                    className="flex flex-col items-end gap-1.5 z-10"
                  ...
                  >
                    ...
                    <a
                      ...
                      className="flex items-center gap-1 text-[11px] font-bold text-text-muted hover:text-primary-main transition-colors bg-bg-secondary px-2 py-1 rounded-md mt-auto"
                    >
                ...
                <div className="grid grid-cols-3 gap-2 mt-3 ml-2">
                  ...
                  <div className="bg-bg-secondary/50 rounded-lg p-2 text-center border border-border-subtle/30 flex items-center justify-center gap-2">
                    ...
                    <span className="text-sm font-bold text-primary-main leading-none">
        ...
        <form onSubmit={handleSave} className="space-y-8">
          <div className="flex items-center gap-6 p-6 bg-bg-secondary rounded-2xl border border-border-subtle border-dashed">
            <label className="w-20 h-20 rounded-full bg-surface-card border-2 border-primary-main/10 flex items-center justify-center text-text-muted relative group cursor-pointer hover:border-primary-main transition-all overflow-hidden">
              {avatarPreview ? (
                ...
                <Camera size={24} className="opacity-40 group-hover:scale-110 transition-all" />
              )}
              <input
                type="file"
                className="hidden"
            ...
            <div className="space-y-1">
              ...
              <p className="text-[13px] text-text-muted font-medium">
                Click the circle to browse or take photo
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <SelectField
              ...
              options={[
                { value: '0.5', label: 'Tier 0.5%' },
                { value: '1.0', label: 'Tier 1.0%' },
                { value: '1.5', label: 'Tier 1.5%' },
              ]}
      ...
      {filtered.length === 0 && (
        <div className="bg-surface-card rounded-2xl shadow-sm border border-border-subtle border-dashed text-center py-32 flex flex-col items-center justify-center gap-6">
          ...
          <p className="text-text-muted font-medium text-[11px] opacity-40 italic">
            No personnel records detected in the active registry.
          </p>
        </div>
      )}
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-5 p-5 bg-bg-secondary rounded-2xl border border-border-subtle">
              <div className="w-16 h-16 rounded-2xl bg-surface-card border border-border-subtle flex items-center justify-center overflow-hidden shadow-inner text-lg font-bold text-primary-main">
                {viewingStaff.avatar ? (
                  <img src={viewingStaff.avatar} className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-1">
                  <Badge variant="primary" className="text-[11px]">
                    {viewingStaff.role?.replace(/_/g, ' ')}
                  </Badge>
                  {viewingStaff.isOnline ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="bg-surface-card border border-border-subtle rounded-xl p-3 text-center">
                <p className="text-[11px] text-text-muted uppercase font-bold tracking-widest mb-1">
                  Age
                </p>
                <p className="text-xl font-bold text-text-main">
                  {calculateAge(viewingStaff.dateOfBirth)}
                </p>
              </div>
              <div className="bg-surface-card border border-border-subtle rounded-xl p-3 text-center">
                <p className="text-[11px] text-text-muted uppercase font-bold tracking-widest mb-1">
                ...
                <p className="text-xl font-bold text-text-main">{viewingStaff.activeInspections}</p>
              </div>
              <div className="bg-surface-card border border-border-subtle rounded-xl p-3 text-center">
                <p className="text-[11px] text-text-muted uppercase font-bold tracking-widest mb-1">
                  Completed
                </p>
                <p className="text-xl font-bold text-text-main">{viewingStaff.totalDealsClosed}</p>
              </div>
              <div className="bg-surface-card border border-border-subtle rounded-xl p-3 text-center">
                <p className="text-[11px] text-text-muted uppercase font-bold tracking-widest mb-1">
                  Rating
                </p>
                <p className="text-xl font-bold text-primary-main">{viewingStaff.averageRating}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-3 p-3 bg-bg-secondary/50 rounded-xl">
                <Phone size={14} className="text-primary-main" />
                <div>
                  <p className="text-[11px] text-text-muted uppercase font-bold tracking-widest">
                    Phone
                  </p>
                  <p className="text-xs font-bold text-text-main">{viewingStaff.phone}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 p-3 bg-bg-secondary/50 rounded-xl">
