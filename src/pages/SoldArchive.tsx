      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Tooltip content="Total net profit after unit costs">
          <KpiTile
            ...
            icon={<DollarSign size={14} />}
            className="p-6 h-32 text-success-main"
          />
        </Tooltip>
          ...
          <table className="w-full text-left border-separate border-spacing-y-2 px-4">
            ...
            <tbody className="space-y-4">
              {archive.map((car) => (
                <tr
                  key={car.id}
                  onClick={() => navigate(`/archive/${car.id}`)}
                ...
                >
                  <td className="py-4 px-4 bg-bg-secondary/30 border-y border-l border-border-subtle/30 rounded-l-2xl group-hover:bg-bg-secondary/50 transition-all">
                    <div className="flex items-center gap-5">
                      ...
                      <div className="space-y-1">
                        <p className="text-text-main font-bold text-sm tracking-tight leading-tight">
                          {car.name}
                        </p>
                        <Badge
                          variant="default"
                          className="font-mono text-[12px] text-text-muted/60"
                        >
                          {car.plate}
                        </Badge>
                      </div>
                  ...
                  <td className="py-4 px-4 bg-bg-secondary/30 border-y border-r border-border-subtle/30 rounded-r-2xl text-right group-hover:bg-bg-secondary/50 transition-all">
                    <div className="space-y-1 text-right">
                      ...
                      <Badge
                        variant="success"
                        className="bg-success-main/10 text-success-main border-none"
                      >
        ...
        <div className="md:hidden flex flex-col gap-4 p-4">
          {archive.map((car) => (
            <div
              key={car.id}
              onClick={() => navigate(`/archive/${car.id}`)}
              className="bg-surface-card rounded-2xl shadow-sm border border-border-subtle/30 transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 p-5 relative overflow-hidden group cursor-pointer active:scale-[0.98]"
            >
              <div className="flex items-start gap-4 mb-4">
                ...
                <div className="space-y-1 flex-1">
                  <h3 className="text-sm font-bold text-text-main tracking-tight leading-tight">
                    {car.name}
                  </h3>
                  <Badge
                    variant="default"
                    className="font-mono text-[12px] bg-bg-secondary border-border-subtle/30 text-text-muted/60"
                  >
                    {car.plate}
                  </Badge>
              ...
              <div className="bg-bg-secondary/30 rounded-xl p-3 border border-border-subtle/30 flex items-center justify-between mb-4">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs font-bold text-text-main">
                    {(car.price / 1000000).toFixed(2)}M ETB
                  </p>
                </div>
                <div className="space-y-0.5 text-right">
                  <p className="text-[12px] text-error-main font-medium">Unit Cost</p>
                  <p className="text-xs font-bold text-text-main">
                    {(car.unitCost / 1000000).toFixed(2)}M ETB
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-border-subtle/30 pt-3">
                ...
                <div className="text-right">
                  <p className="text-lg font-black text-success-main tracking-tight leading-none">
                    +{(car.profit / 1000000).toFixed(2)}M
                  </p>
